""" Module with functionalities for reading data_io from a file and return a
    dictionary with record identifiers as keys and a list of attribute values.

    Also provides a function to load a truth data_io set of record pairs that are
    matches.
"""
...
def load_data_set(file_name, rec_id_col, use_attr_list, header_line):
    """Load the data_io set and store in memory as a dictionary with record
     identifiers as keys.

     Parameter Description:
       file_name      : Name of the data_io file to be read (CSV or CSV.GZ file)
       rec_id_col     : Record identifier column of the data_io file
  ...
  """
    ...
    print('Load data_io set from file: ' + file_name)

    if header_line:
        header_list = next(csv_reader)
        print('  Header line: ' + str(header_list))
    header_dict = {}
    for index, col_name in enumerate(header_list):
        header_dict[col_name] = index
    ...
    if len(rec_dict) < rec_num:
        print('  *** Warning, data_io set contains %d duplicates ***' % \
...
def load_truth_data(file_name):
    """Load a truth data_io file where each line contains two record identifiers
  ...
  """
    ...
    print('Load truth data_io from file: ' + file_name)
